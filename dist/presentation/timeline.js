...
var Timeline = (function (_PlayerObject) {
    ...
    _inherits(Timeline, _PlayerObject);
    ...
    _createClass(Timeline, {
        update: {
            ...
            value: function update() {
                var periods = this.presentation.manifest.periods;
                ...
                if (this.presentation.willReloadManifest) {
                    var anyMissingID = periods.some(function (period) {
                        return period.id == undefined;
                    });

                    if (anyMissingID) console.warn("some periods in dynamic manifest are missing " + "an id, proceding on the assumption only one period exists");
                }
                ...
                var firstID = periods[0].id;
                var firstIndex = this.intervals.findIndex(function (i) {
                    return i.id == firstID;
                });

                if (firstIndex == -1 && this.intervals.length > 0) throw "cannot update manifest, no common periods were found";
                ...
                if (this.intervals.length > periods.length) throw "cannot update manifest, new manifest is missing periods";
                ...
                var lastIndex = this.intervals.length - 1;

                for (var i = 0; i < periods.length; i++) {
                    if (i < this.intervals.length) {
                        if (this.intervals[i].id != periods[i].id) throw "cannot update manifest, out of order periods";
                        if (i == lastIndex) this.intervals[i].updateWith(periods[i]);
                    } else {
                        var interval = new Interval(this.presentation, periods[i]);
                ...
                var finalInterval = this.intervals[this.intervals.length - 1];

                if (manifest.mediaPresentationDuration) this.duration = manifest.mediaPresentationDuration;else if (finalInterval.duration != undefined) this.duration = finalInterval.end;else this.duration = undefined;

                // ensure a currentInterval is always available
                if (this.currentInterval == undefined) this.currentInterval = this.intervals[0];
            }
        },
        calculateStartAndDuration: {

            // ---------------------------
            // timing
            // ---------------------------

            value: function calculateStartAndDuration(updatedIntervals) {
                ...
                for (var i = 0; i < updatedIntervals.length; i++) {
                    var interval = updatedIntervals[i];

                    if (interval.period.start != undefined) interval.start = interval.period.start;else if (i == 0 && interval.start == undefined) interval.start = 0;

                    if (interval.period.duration != undefined) interval.duration = interval.period.duration;else interval.duration = interval.contentDerivedDuration();
                }
                ...
                var lastIndex = updatedIntervals.length - 1;
                for (var i = 0; i < updatedIntervals.length; i++) {
                    var interval = updatedIntervals[i];

                    if (interval.start == undefined) {
                        var previous = updatedIntervals[i - 1];
                        if (!previous.duration) throw "cannot calculate interval start";
                    ...
                    if (interval.duration == undefined) {
                        ...
                        if (i == lastIndex) {
                            var manifest = this.presentation.manifest;
                            if (manifest.mediaPresentationDuration) interval.duration = manifest.mediaPresentationDuration - interval.start;
                        } else {
                            var next = updatedIntervals[i + 1];
                            if (!next.start) throw "cannot calculate interval duration";
        ...
        intervalAt: {
            value: function intervalAt(time) {
                return this.intervals.find(function (interval) {
                    return interval.start <= time && (time < interval.end || interval.end == undefined);
                });
            }
        },
        videoSegmentAt: {
            ...
            value: function videoSegmentAt(time) {
                return this.intervalAt(time).videoContent.segmentAt(time);
            }
        },
        audioSegmentAt: {
            value: function audioSegmentAt(time) {
                return this.intervalAt(time).audioContent.segmentAt(time);
            }
        },
        videoSegmentsInRange: {
            value: function videoSegmentsInRange(start, end) {
                return this.intervalAt(start).videoContent.segmentsInRange(start, end);
            }
        },
        audioSegmentsInRange: {
            value: function audioSegmentsInRange(start, end) {
